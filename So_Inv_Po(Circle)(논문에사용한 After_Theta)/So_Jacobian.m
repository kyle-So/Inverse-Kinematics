function [ Structure_Jaco ] = So_Jacobian( Theta )
...
% i_Link = 현재의 링크 즉 A1, A12, A123 링크중 만약 C_Link=3 이라면 A123을 쓰고있는거임
        ...
        for Num_mid = 1: length(mid_point)
            ...
            if i_Link == 1
            Structure_Jaco{i_Link,Num_mid} =  [Sum_J{i_Link}*mid_point(Num_mid)] ;
            else
            Structure_Jaco{i_Link,Num_mid} = [( ( Sum_J{i_Link}-Sum_J{i_Link-1} )*mid_point(Num_mid) ) + Sum_J{i_Link-1}];    
            end
        end
...
% Structure_Jaco{7,2}
%=================================================================================
        % 이 부분은 2 by 2, 3by3 ....n by n 자코비안또는 6by7를 만드는 부분이지만
        % 위에서 속도텀만 쓰므로 항상 3 by m(링크수) 자코비안을 만들기에 필요없는부분
        % 즉 위에서 Structure_Jaco 가 이미 3 by m 자코비안임
